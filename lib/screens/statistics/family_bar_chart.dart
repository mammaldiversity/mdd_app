...
class FamilyBarChart extends StatefulWidget {
  const FamilyBarChart({super.key, required this.stats});

  final MddStatistics stats;

  @override
  State<FamilyBarChart> createState() => _FamilyBarChartState();
}

class _FamilyBarChartState extends State<FamilyBarChart> {
  int _topN = 15;

  @override
  Widget build(BuildContext context) {
    if (widget.stats.speciesPerFamily.isEmpty) return const SizedBox.shrink();
    final List<StatSpeciesPerFamilyResult> data =
        widget.stats.speciesPerFamily.take(_topN).toList();
    final double maxY =
