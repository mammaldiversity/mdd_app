enum TextTokenType { standard, italic, url }

class TextToken {
  const TextToken({required this.text, required this.type, this.url});
  ...
  static List<TextToken> parse(String? rawText) {
    ...
    for (final match in exp.allMatches(text)) {
      ...
      if (matchText.startsWith('http')) {
        tokens.add(
          TextToken(text: matchText, type: TextTokenType.url, url: matchText),
        );
