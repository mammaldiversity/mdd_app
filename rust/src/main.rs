...
fn main() {
    ...
    let mil_helper = if mil_tar_path.ends_with(".json") {
        ...
        for i in 0..archive.len() {
            ...
            if file_name.contains("MDD_v") && file_name.ends_with(".csv") && !file_name.contains("__MACOSX") {
                let mut out = std::fs::File::create(&temp_csv_path).expect("Failed to create temp CSV");
        ...
        mdd_api::mil::prepare_metadata(
            std::path::Path::new(mil_tar_path),
            &temp_csv_path,
            None,
            &temp_json_path,
        ...
        let mut json_file = std::fs::File::open(&temp_json_path).expect("Failed to open prepared MIL JSON");
        use std::io::Read;
        json_file.read_to_string(&mut json_content).expect("Failed to read prepared MIL JSON");

        MilHelper { mil_data: json_content }
    };
